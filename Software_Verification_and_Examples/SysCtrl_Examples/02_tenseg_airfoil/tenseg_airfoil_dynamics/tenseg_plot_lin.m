function [fig_out] = tenseg_plot_lin( N,C_b,C_s,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata)
...
if ~isempty(R3Ddata) && isfield(R3Ddata,'Nradius') % 3D plot
    ...
    plot3(N(1,:),N(2,:),N(3,:),'ko','MarkerSize',4,'MarkerFaceColor','black')
end
...
if LabelNodes == 1
    for i = 1:size(N,2)
        text(N(1,i) + dist_x, N(2,i) + dist_y, N(3,i) + dist_z,...
            num2str(i), 'FontSize', FontNodes, 'Color', 'k')
    end
end
...
%     plot3(N(1,node_index),N(2,node_index),N(3,node_index),'rd','MarkerSize',8,'MarkerFaceColor','red')
    plot3(N(1,node_index),N(2,node_index),N(3,node_index),'ko','MarkerSize',4,'MarkerFaceColor','black')
