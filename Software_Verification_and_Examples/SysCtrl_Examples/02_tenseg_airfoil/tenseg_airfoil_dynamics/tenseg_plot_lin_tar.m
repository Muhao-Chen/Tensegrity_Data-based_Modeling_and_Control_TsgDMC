function [fig_out] = tenseg_plot_lin_tar( N,C_b,C_s,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata)
...
BarWidth = 0.01; % Width of bar lines
StringWidth = 0.01; % Width of string lines
    ...
    if LabelBars == 1
        for i = 1:size(B,2)
            text(bar_start_nodes(1,i) + 0.5*B(1,i) + dist_x, ...
                ...
                num2str(i), 'FontSize',FontBars, 'Color', 'b')
        end
    end
end
    ...
    if LabelStrings == 1
        for i = 1:size(S,2)
            text(string_start_nodes(1,i) + 0.45*S(1,i) + dist_x, ...
                ...
                num2str(i), 'FontSize',FontStrings,'color','[1 0.7529 0.7961].','Autoscale','off','LineWidth',2,'ShowArrowHead','off');
...
if ~isempty(R3Ddata) && isfield(R3Ddata,'Nradius') % 3D plot
    ...
    plot3(N(1,:),N(2,:),N(3,:),'bo','MarkerSize',4,'MarkerFaceColor','blue')
end
...
%     plot3(N(1,node_index),N(2,node_index),N(3,node_index),'rd','MarkerSize',8,'MarkerFaceColor','red')
    plot3(N(1,node_index),N(2,node_index),N(3,node_index),'bo','MarkerSize',4,'MarkerFaceColor','blue')
end
