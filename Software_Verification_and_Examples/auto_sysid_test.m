...
testFileList={'main_pendulum_sysID.m',...
              'main_tsg_airfoil_sysID.m',...
              'main_beam_sysID.m',...
              };
...
for q_test=1:1:numel(testFileList)
    ...
    fileMessage=['testSysID -> Test file: ',num2str(q_test),' of ',num2str(numel(testFileList)),' ',testFileList{q_test}];
    disp(' ');
    disp(fileMessage);
    disp(' ');

    % Make testFolder current directory
    testFolder=fullfile(fileparts(mfilename('fullpath')),'SysID_Examples',testFolderList{q_test});
    addpath(testFolder);
    cd(testFolder);
